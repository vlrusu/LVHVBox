...
float i2c_ltc2497(int address, int channelLTC) {
  ...
  uint32_t val =
      ((block[0] & 0x3f) << 16) + (block[1] << 8) + (block[2] & 0xE0);
...
void reset_trip(uint8_t channel, int client_addr) {
  ...
  if (channel < 6) {
    libusb_bulk_transfer(device_handle_0, 0x02, &send_val, sizeof(send_val), 0,
                         0);
  } else {
    libusb_bulk_transfer(device_handle_1, 0x02, &send_val, sizeof(send_val), 0,
                         0);
  }
    ...
    if (allow_read == 1) {
      ...
      for (uint8_t channel = 0; channel < 6; channel++) {
        for (uint16_t time = 0; time < HISTORY_LENGTH; time++) {
          memcpy(&common->all_currents_stored[channel][time],
                 &common->all_currents[channel][time],
                 sizeof(&common->all_currents[channel][time]));
        }
      }
      ...
      if (pico == 0) {
        ...
        libusb_bulk_transfer(device_handle_0, 0x82, voltage_input_data, 24, 0,
                             0);
        ...
        libusb_bulk_transfer(device_handle_0, 0x02, &current_stop_char, 1, 0,
        0);


        libusb_bulk_transfer(device_handle_0, 0x02, &full_current_char, 1, 0,
        0);

        libusb_bulk_transfer(device_handle_0, 0x82, full_current_data, 60, 0,
        0);
        */
        ...
        libusb_bulk_transfer(device_handle_1, 0x82, voltage_input_data, 24, 0,
                             0);
        usb1_lock = 0;
      }
...
void *save_txt(void *arguments) {
  ...
  while (1) {
    ...
    if (array_indicator != old_array_indicator) {
      ...
      for (uint8_t channel = 0; channel < 6; channel++) {
        for (uint16_t time = 0; time < HISTORY_LENGTH; time++) {
          memcpy(&store_all_currents_internal[channel][time],
                 &common->all_currents_stored[channel][time],
                 sizeof(&common->all_currents_stored[channel][time]));
        }
      }
      ...
      if (pico == 0) {
        ...
        for (int pipe_id = 0; pipe_id < num_pipes; pipe_id++) {
          for (uint32_t time_index = 0; time_index < HISTORY_LENGTH;
               time_index++) {
            for (uint8_t channel = 0; channel < 6; channel++) {
              // Apply the first-order low-pass filter
              float input_value =
                  store_all_currents_internal[channel][time_index];
              float filtered_value =
                  ALPHA * input_value + (1 - ALPHA) * last_output[channel];
              ...
              if (time_index % DECIMATION_FACTOR == 0 && use_pipe == 1) {
                int length =
                    snprintf(buffer, sizeof(buffer), "%f ", filtered_value);
        ...
        /*
              ...
              for (uint32_t time_index=0; time_index<HISTORY_LENGTH;
           time_index++) { for (uint8_t channel=0; channel<6; channel++) {
                  fprintf(fp_I, "%f ",
           store_all_currents_internal[channel][time_index]);
                }
                seconds = time(NULL);
