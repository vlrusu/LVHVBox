...
static const char *spidev =
    "/dev/spidev0.0";  // this is the SPI device. Assume here that there is only
                       // one SPI bus
...
#define ALPHA \
  0.1  // Choose a value between 0 and 1. Smaller values result in heavier
       // filtering.
#define DECIMATION_FACTOR 10
uint8_t use_pipe = 1;  // when server tries to open pipe, will be set to 0 upon
                       // fail - will then assume pipe is not to be used
int num_pipes = 6;
...
command parse_pico_queue(int pico) {
  ...
  if (pico == 0) {
    msgrcv(pico0_msqid, (void *)&return_command, 56, msgtyp,
           MSG_NOERROR | IPC_NOWAIT);
  } else {
    msgrcv(pico1_msqid, (void *)&return_command, 56, msgtyp,
           MSG_NOERROR | IPC_NOWAIT);
  }
  return return_command;
}
...
float i2c_ltc2497(int address, int channelLTC) {
  ...
  if (write(lv_i2c, block, length) !=
      length)  // write() returns the number of bytes actually written, if it
               // doesn't match then an error occurred (e.g. no response from
               // the device)
  {
  ...
  if (read(lv_i2c, block, length) !=
      length)  // read() returns the number of bytes actually read, if it
               // doesn't match then an error occurred (e.g. no response from
               // the device)
  {
    // ERROR HANDLING: i2c transaction failed
  ...
  if ((block[0] & 0x80)) {
    val =
        (int)(((block[0] & 0x3f) << 16) + (block[1] << 8) + (block[2] & 0xE0));
  } else {
    val = (int)(((~block[0] & 0x3f) << 16) + (~block[1] << 8) +
                (~block[2] & 0xE0));
  }
...
void set_hv(int channel, float value) {
  ...
  float alphas[12] = {0.9,    0.9,    0.885,  0.9,    0.9012, 0.9034,
                      0.9009, 0.9027, 0.8977, 0.9012, 0.9015, 1.};

  uint32_t digvalue =
      ((int)(alphas[channel] * 16383. * (value / 1631.3))) & 0x3FFF;
...
void get_buffer_status(uint8_t channel, int client_addr) {
  ...
  if (0 <= channel && channel < 6 && use_pico0 == 1) {
    ...
    libusb_bulk_transfer(device_handle_0, 0x82, input_data, sizeof(input_data),
                         0, 0);
    ...
    libusb_bulk_transfer(device_handle_1, 0x82, input_data, sizeof(input_data),
                         0, 0);
...
void trip_status(uint8_t channel, int client_addr) {
  ...
  if (client_addr != -9999) {
    ...
    write_fixed_location(LIVE_STATUS_FILENAME, 2 * channel,
                         return_val);  // writes at fixed location
  }
}
...
void set_trip(uint8_t channel, float value, int client_addr) {
  ...
  if (0 <= channel && channel < 6 && use_pico0 == 1) {
    pthread_mutex_lock(&usb0_mutex_lock);
    libusb_bulk_transfer(device_handle_0, 0x02, &send_val[0], sizeof(send_val),
                         0, 0);
    pthread_mutex_unlock(&usb0_mutex_lock);
  } else if (5 < channel && channel < 12 && use_pico1 == 1) {
    pthread_mutex_lock(&usb1_mutex_lock);
    libusb_bulk_transfer(device_handle_1, 0x02, &send_val[0], sizeof(send_val),
                         0, 0);
    pthread_mutex_unlock(&usb1_mutex_lock);
  } else {
...
void set_trip_count(uint8_t channel, float value, int client_addr) {
  ...
  char *command_log = load_config("Command_Log_File");
  char log_message[50];
  sprintf(log_message, "set_trip_count ch%u %i triggers", channel,
          new_requirement);
  write_log(command_log, log_message, client_addr);
  ...
  while (1) {
    ...
    if (return_val == 13) {
      ...
      if (add_command.command_type == TYPE_pico &&
          ((uint8_t)add_command.char_parameter < 6 |
           add_command.command_name == COMMAND_pcb_temp)) {
        msgsnd(pico0_msqid, (void *)&add_command, sizeof(add_command),
               IPC_NOWAIT);
      } else if (add_command.command_type == TYPE_pico) {
        msgsnd(pico1_msqid, (void *)&add_command, sizeof(add_command),
               IPC_NOWAIT);
      } else if (add_command.command_type == TYPE_hv ||
                 add_command.command_type == TYPE_lv) {
        int snd_success =
            msgsnd(msqid, (void *)&add_command, sizeof(add_command), 0);
      }
    }
  }
}
...
void *call_ped(void *args) {
  while (1) {
    ...
    msgsnd(pico0_msqid, (void *)&call_ped_command, sizeof(call_ped_command),
           IPC_NOWAIT);
    msgsnd(pico1_msqid, (void *)&call_ped_command, sizeof(call_ped_command),
           IPC_NOWAIT);
  }
}
...
int write_pipe_currents(int fd[num_pipes],
    ...
    for (uint32_t time_index = 0; time_index < HISTORY_LENGTH; time_index++) {
      for (uint8_t channel = 0; channel < 6; channel++) {
        ...
        float filtered_value =
            ALPHA * input_value + (1 - ALPHA) * last_current_output[channel];
      ...
      if (time_index % DECIMATION_FACTOR == 0 && use_pipe == 1) {
        ...
        write_success = write(fd[pipe_id], buffer,
                              length);  // Write the timestamp to the pipe
  ...
  if (pico == 0) {
    ...
    for (int channel = 0; channel < 6; channel++) {
      ...
      for (int pipe_id = 0; pipe_id < num_pipes; pipe_id++) {
        if (use_pipe == 1) {
          int length =
              snprintf(buffer, sizeof(buffer), "%f ", voltages_0[channel]);
          write_success =
...
int initialize_libusb(int pico) {
  ...
  if (pico == 0) {
    device_handle_0 =
        libusb_open_device_with_vid_pid(NULL, VENDOR_ID_0, PRODUCT_ID);
    ...
    device_handle_1 =
        libusb_open_device_with_vid_pid(NULL, VENDOR_ID_1, PRODUCT_ID);
...
void *acquire_data(void *arguments) {
  ...
  while (1) {
    ...
    if (current_type == TYPE_pico) {
      ...
      if (current_command == COMMAND_trip) {  // trip
        trip(char_parameter, client_addr);
      } else if (current_command == COMMAND_reset_trip) {  // reset trip
        reset_trip(char_parameter, client_addr);
      } else if (current_command == COMMAND_disable_trip) {  // disable trip
        disable_trip(char_parameter, client_addr);
      } else if (current_command == COMMAND_enable_trip) {  // enable trip
        enable_trip(char_parameter, client_addr);
      } else if (current_command == COMMAND_set_trip) {  // set trip
        set_trip(char_parameter, float_parameter, client_addr);
      } else if (current_command == COMMAND_set_trip_count) {  // set trip count
        set_trip_count(char_parameter, float_parameter, client_addr);
      } else if (current_command == COMMAND_trip_status) {  // trip status
      ...
      } else if (current_command == COMMAND_enable_ped) {  // enable ped
        enable_ped(char_parameter, client_addr);
      } else if (current_command == COMMAND_disable_ped) {  // disable ped
    ...
    if (pause_usb == 0) {
      if (pico == 0 && use_pico0 == 1) {
        int current_success_0 =
            request_averaged_currents(common->all_currents, 0);
        int voltage_success_0 = request_voltages(0);
      } else if (use_pico1 == 1) {
        int current_success_1 =
            request_averaged_currents(common->all_currents, 1);
      ...
      if (pico == 0 && use_pico0 == 1) {
        ...
        int pipe_current_success =
            write_pipe_currents(fd, common->all_currents);
...
int lv_initialization() {
  ...
  for (int i = 0; i < 6; i++) {
    if (MCP_pinMode(lvpgoodMCP, powerChMap[i], OUTPUT) == -1) {
      sprintf(error_msg, "lv_initialization MCP_pinMode pin %u failure",
              powerChMap[i]);
      error_log(error_msg);
      printf(error_msg);
    ...
    if (MCP_pinWrite(lvpgoodMCP, powerChMap[i], 0) == -1) {
      sprintf(error_msg, "lv_initialization MCP_pinWrite pin %u failure",
              powerChMap[i]);
      error_log(error_msg);
      printf(error_msg);
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 6; i++) {
    powerChMap[i] = (uint8_t)(prepowerchmap[i] -= '0');
  }
