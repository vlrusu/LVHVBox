  ...
  if ( tud_cdc_available() )
    {
      ...
      } else if (receive_chars[0] > 96 && receive_chars[0] < 103) { // ----- start/stop current buffer ----- //

        *current_buffer_run = ~*current_buffer_run;

      } else if (receive_chars[0] > 114 && receive_chars[0] < 121) { // ----- Send chunk of current burst ----- //
        float temp_current;
...
void get_all_averaged_currents(PIO pio_0, PIO pio_1, uint sm[], float average_current_array[6], float full_current_array[6]) {
 ...
 {
  average_current_array[channel] = average_current_array[channel]*adc_to_uA/200 + 8.3;
 }
  ...
  for (uint32_t i=0; i<6; i++) {
        ...
        if ((full_current_array[i] > trip_currents[i]) && ((trip_mask & (1 << i)))) {
...
gpio_put(all_pins.P1_0, 1); // put pedestal pin high
  ...
  while (true) // DAQ & USB communication Loop, runs forever
  {
    ...
    for (uint32_t i=0; i<10; i++) {
      get_all_averaged_currents(pio_0, pio_1, sm_array, channel_current_averaged, full_current); // get average of 200 full speed current measurements
      ...
      cdc_task(channel_current_averaged, channel_voltage, burst_current, sm_array, &burst_position, trip_currents, &trip_mask, &trip_status, average_current_history, &average_store_position, full_current_history, &full_position, &current_buffer_run);
