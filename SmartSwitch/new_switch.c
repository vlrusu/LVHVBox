...
float trip_currents[6] = {180, 180, 180, 180,
                          180};  // tripping threshold currents
...
uint32_t full_current_history_length = 2000;
uint16_t current_buffer_run = 1;
...
void get_current_burst(
    PIO pio, uint sm,
    int16_t current_array[size_current_burst])  // gets sequential current
                                                // values from PIO at full speed
...
void cdc_task(float channel_current_averaged[6], float channel_voltage[6],
              int16_t burst_current[size_current_burst], uint sm[6],
              ...
              float full_current_history[6][full_current_history_length],
              uint16_t* full_position, uint16_t* current_buffer_run) {
  if (tud_cdc_available()) {
    ...
    } else if (receive_chars[0] > 96 &&
               receive_chars[0] <
                   103) {  // ----- start/stop current buffer ----- //

      *current_buffer_run = ~*current_buffer_run;

    } else if (receive_chars[0] > 114 &&
               receive_chars[0] <
                   121) {  // ----- Send chunk of current burst ----- //
      float temp_current;
      int channel = receive_chars[0] - 115;

      for (uint8_t i = 0; i < 15; i++) {
        temp_current =
            full_current_history[channel][*full_position + i] * adc_to_uA;
        tud_cdc_write(&temp_current, sizeof(&temp_current));
      }
      tud_cdc_write_flush();

      *full_position += 15;
      if (*full_position > full_current_history_length - 1) {
        *full_position = *full_position - full_current_history_length;
      }
...
void get_all_averaged_currents(PIO pio_0, PIO pio_1, uint sm[],
                               float average_current_array[6],
                               float full_current_array[6]) {
  for (uint32_t channel = 0; channel < 6;
       channel++)  // initializes each element of current array to zero
  {
    average_current_array[channel] = 0;
  }
       ...
       i++)  // adds current measurements to current_array
  {
    for (uint32_t channel = 0; channel < 3; channel++) {
      ...
      average_current_array[channel] += latest_current_0;
      average_current_array[channel + 3] += latest_current_1;

      // update latest full current, along with its position in rotating buffer
      full_current_array[channel] = latest_current_0;
      full_current_array[channel + 3] = latest_current_1;
    }
  }
  ...
  for (uint32_t channel = 0; channel < 6;
       channel++)  // divide & multiply summed current values by appropriate
                   // factors
  {
    average_current_array[channel] =
        average_current_array[channel] * adc_to_uA / 200 + 8.3;
  }

  for (uint32_t i = 0; i < 6; i++) {
    // check if trip is required
    if ((full_current_array[i] > trip_currents[i]) &&
        ((trip_mask & (1 << i)))) {
      if (num_trigger[i] > 80) {
...
#define PICO_XOSC_STARTUP_DELAY_MULTIPLIER 64
  ...
  set_sys_clock_khz(
      210000,
      true);  // Overclocking just to be sure that pico keeps up with everything
  // not aware of any real drawbacks, SmartSwitch can probably keep up even
  // without overclocking So if it becomes an issue, overclocking is probably
  // unecessary
  ...
  float full_current[6] = {0, 0, 0, 0, 0, 0};
  ...
  while (true)  // DAQ & USB communication Loop, runs forever
  {
    ...
    for (uint32_t i = 0; i < 10; i++) {
      get_all_averaged_currents(
          ...
          full_current);  // get average of 200 full speed current measurements
          ...
          1999)  // check that current buffer size has not been exceeded
      {
        ...
        if (current_buffer_run == 1) {
          for (uint8_t i = 0; i < 6; i++) {
            full_current_history[i][full_position] = full_current[i];
          }
          full_position += 1;
        }
      ...
      cdc_task(channel_current_averaged, channel_voltage, burst_current,
               sm_array, &burst_position, trip_currents, &trip_mask,
               &trip_status, average_current_history, &average_store_position,
               full_current_history, &full_position, &current_buffer_run);
    ...
    cdc_task(channel_current_averaged, channel_voltage, burst_current, sm_array,
