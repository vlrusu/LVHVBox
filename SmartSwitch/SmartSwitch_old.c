    ...
    if (102 < receive_chars[0] &&
      ...
      *trip_status =
          *trip_status &
          ~(1
            << receive_chars[0] -
                   109);  // store information that channel is no longer tripped
      sleep_ms(250);
      *trip_mask = *trip_mask | (1 << (receive_chars[0] - 109));
      ...
      *current_buffer_run = 0;
      ...
      // currently not accounting for pedestal
       for (int i=0; i<10; i++) {
        if (ped_on == 1) {
          temp_currents[i] = full_current_history[channel][*full_position + i] *
      adc_to_uA - ped_subtraction[channel]; } else { temp_currents[i] =
...
void get_all_averaged_currents(
    ...
    uint32_t full_current_array[6][full_current_history_length],
    uint16_t* full_position, uint8_t* current_buffer_run,
    int* remaining_buffer_iterations) {
       ...
       i++)  // adds current measurements to current_array
  {
    for (uint32_t channel = 0; channel < 3; channel++) {
      ...
      if ((latest_current_0 * adc_to_uA > trip_currents[channel]) |
          (latest_current_1 * adc_to_uA > trip_currents[channel + 3])) {
        if ((trip_mask & (1 << channel)) && *current_buffer_run == 1) {
          if (num_trigger[channel] > trip_requirement) {
            *current_buffer_run = 0;
            *remaining_buffer_iterations =
                floor(full_current_history_length / 2);
            ...
            for (int channel_index = 0; channel_index < 6; channel_index++) {
              for (int current_index = 0;
                   current_index < full_current_history_length;
                   current_index++) {
                present_current =
                    full_current_array[channel_index][current_index] *
                    adc_to_uA;
                if (present_current > max_current &&
                    ((~trip_status & (1 << channel_index)))) {
                  memcpy(&max_current, &present_current,
                         sizeof(present_current));
      ...
      for (int ped_count = 0; ped_count < 5000; ped_count++) {
        for (int i = 0; i < 3; i++) {
          pre_ped_subtraction[i] +=
              (uint16_t)pio_sm_get_blocking(pio_0, sm_array[i]);
          pre_ped_subtraction[i + 3] +=
              (uint16_t)pio_sm_get_blocking(pio_1, sm_array[i + 3]);
        }
      }
    ...
    for (int j = 0; j < 35; j++) {
      ...
      for (uint32_t i = 0; i < 1000; i++) {
        get_all_averaged_currents(
            ...
            &remaining_buffer_iterations);  // get average of 200 full speed
