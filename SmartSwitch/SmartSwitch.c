...
SMArray sm_init() {
  float clkdiv = 45;  // clock divider for PIO, results in 81.967 kHz
...
void cdc_task(float channel_current_averaged[6], float channel_voltage[6],
  ...
  if (102 < receive_chars[0] &&
    ...
    *current_buffer_run = 1;
    ...
    uint8_t tripPin =
        all_pins
            .crowbarPins[receive_chars[0] - 109];  // acquire proper crowbar pin
...
void core1_entry() {
  ...
  while (1) {
    ...
    // ----- Collect average current measurements ----- //
    for (uint32_t i = 0; i < 1000; i++) {
      // get average of 200 full speed current measurements
...
int main() {
  ...
  uint16_t burst_position = 0;  // completely unused

  // ensure that all crowbar pins are initially off
  for (uint8_t i = 0; i < 6; i++) {
  ...
  while (true) {
    cdc_task(channel_current_averaged, channel_voltage, &burst_position,
             trip_currents, &trip_mask, &trip_status, average_current_history,
             &average_store_position, full_current_array, &full_position,
             &current_buffer_run, &slow_read, &before_trip_allowed,
             trip_requirement);
    tud_task();
  }
  return 0;
