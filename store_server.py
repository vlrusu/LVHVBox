...
class ClientThread(Thread):
    ...
    def run(self):
        while True:
            ...
            print("message header: " + str(message_header.decode('utf-8').strip()))
            message_length = int(message_header.decode('utf-8').strip())
            print(message_length)
            data = json.loads(self.conn.recv(message_length).decode('utf-8'))

            command  = Command(1, data, self.conn)
            print (f"Server received data: {data}")
            if not incoming_queue.full() and type(data) is dict:
                if data["type"] == 0:
...
while True:
    c, addr = tcpServer.accept()  # Establish connection with client.
